...
import edu.uiuc.ncsa.security.util.qdl.statements.ConditionalStatement;
import edu.uiuc.ncsa.security.util.qdl.statements.Element;
import edu.uiuc.ncsa.security.util.qdl.statements.Statement;
import edu.uiuc.ncsa.security.util.qdl.statements.WhileLoop;
...
public class QDLListener implements QDLParserListener {
    ...
    public QDLListener(ParsingMap parsingMap,
                       SymbolStack symbolStack,
                       OpEvaluator opEvaluator,
                       FunctionEvaluator functionEvaluator) {
        this.parsingMap = parsingMap;
    ...
    public void enterScalar(QDLParserParser.ScalarContext ctx) {
        ...
        stash(ctx, new VariableNode(symbolStack));
    }
    ...
    public void enterAssignment(QDLParserParser.AssignmentContext ctx) {
        stash(ctx, new Assignment(symbolStack));
    }
    ...
    public void enterFunction(QDLParserParser.FunctionContext ctx) {
        stash(ctx, new Polyad(opEvaluator, functionEvaluator));
    }
    ...
    public void exitFunction(QDLParserParser.FunctionContext ctx) {
        ...
        polyad.setName(name);
        polyad.setOperatorType(functionEvaluator.getType(name));
    ...
    public void exitPrefix(QDLParserParser.PrefixContext ctx) {
        Monad monad = new Monad(opEvaluator, false);
        stash(ctx, monad);

        // in the context, the operator is the 0th child since it is prefixed.
        monad.setOperatorType(opEvaluator.getType(ctx.getChild(0).getText()));
        finish(monad, ctx);
    }
    ...
    public void enterOrExpression(QDLParserParser.OrExpressionContext ctx) {
        stash(ctx, new Dyad(opEvaluator, OpEvaluator.OR_VALUE));
    }
    ...
    public void enterEqExpression(QDLParserParser.EqExpressionContext ctx) {
        stash(ctx, new Dyad(opEvaluator, OpEvaluator.EQUALS_VALUE));
    }
    ...
    public void enterAndExpression(QDLParserParser.AndExpressionContext ctx) {
        stash(ctx, new Dyad(opEvaluator, OpEvaluator.AND_VALUE));
    }
    ...
    public void enterMultiplyExpression(QDLParserParser.MultiplyExpressionContext ctx) {
        stash(ctx, new Dyad(opEvaluator));
    ...
    public void exitMultiplyExpression(QDLParserParser.MultiplyExpressionContext ctx) {
        Dyad dyad = (Dyad) parsingMap.getStatementFromContext(ctx);
        if (ctx.Times() != null) {
            dyad.setOperatorType(OpEvaluator.TIMES_VALUE);
        } else {
            dyad.setOperatorType(OpEvaluator.DIVIDE_VALUE);
        }
        finish(dyad, ctx);
    }
    ...
    public void exitAddExpression(QDLParserParser.AddExpressionContext ctx) {
        Dyad dyad = (Dyad) parsingMap.getStatementFromContext(ctx);
        if (ctx.Minus() != null) {
            dyad.setOperatorType(OpEvaluator.MINUS_VALUE);
        } else {
            dyad.setOperatorType(OpEvaluator.PLUS_VALUE);
    ...
    public void exitCompExpression(QDLParserParser.CompExpressionContext ctx) {
        ...
        dyad.setOperatorType(opEvaluator.getType(ctx.getChild(1).getText()));
        finish(dyad, ctx);
    }

    @Override
    public void enterPostfix(QDLParserParser.PostfixContext ctx) {
        stash(ctx, new Monad(opEvaluator, true));
    }
    ...
    public void exitPostfix(QDLParserParser.PostfixContext ctx) {
        ...
        monad.setOperatorType(opEvaluator.getType(ctx.getChild(1).getText()));
        finish(monad, ctx);
    }
    ...
    protected void doConditional(ParseTree ctx) {
        ...
        //0-th is if[ // first is conditional, 3rd is ]then[. 4th starts the statements
    ...
    protected void doLoop(ParseTree ctx) {
        //a:=3; while[ a < 6 ]do[ q:= 6; say(q + a++); ];
        //a:=3; while[ a < 6 ]do[ q:= 6; say(q+a); ++a;];
        //a:=8; while[ a > 6 ]do[ q:= 6; say(q+a); a--;];
        //a:=8; while[ a > 6 ]do[ q:= 6; say(q + a); --a;];
        WhileLoop whileLoop = (WhileLoop) parsingMap.getStatementFromContext(ctx);
        //0-th is while[ // first is conditional, 2nd is ]do[. 3 starts the statements
