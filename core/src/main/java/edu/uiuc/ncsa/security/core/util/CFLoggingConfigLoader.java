 ...
 */
public abstract class CFLoggingConfigLoader<T extends AbstractEnvironment> implements Serializable, Version, ConfigurationLoader<T> {
    /**
    ...
    public MetaDebugUtil getDebugger() {
        if (debugger == null) {
            ...
            String rawDebug = cn.getFirstAttribute(ConfigurationTags.DEBUG);
            try {
    ...
    public CFLoggingConfigLoader(String defaultFile,
                                 String defaultName,
                                 CFNode node, MyLoggingFacade logger) {
        ...
        List<CFNode> kids = cn.getChildren(LoggingConfigurationTags.LOGGING_COMPONENT);
        //List list = node.getChildren(LoggingConfigurationTags.LOGGING_COMPONENT);
        CFNode currentNode = null;

        if (!kids.isEmpty()) {
            currentNode =  kids.get(0);
        }
    ...
    public CFLoggingConfigLoader(CFNode node, MyLoggingFacade logger) {
        this(null, null, node, logger);

    }

    public CFLoggingConfigLoader(CFNode node) {
        this(node, null);
    }
